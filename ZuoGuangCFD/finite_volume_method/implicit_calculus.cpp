...
namespace zgCFD
{
    namespace imp
    {
        FvmScalarMatrix Diffusion(ScalarVolumeField& gamma, ScalarVolumeField& variable)
        {
            ...
            //            VectorSurfaceField grad_variable_f =
            //                exp::InterpolateGradient(grad_variable, variable, TheInterpolationScheme::AVERAGE);
            ...
            for (uint32_t i = 0; i < variable.mesh_->number_of_interior_faces_; i++)
            {
                ...
                //线性化系数
                Scalar fluxff = gamma_f.interior_value_[i] * variable.mesh_->faces_[i].g_diff_;
                Scalar fluxvf =
                    (grad_variable_f.interior_value_[i] & variable.mesh_->faces_[i].t_) * gamma_f.interior_value_[i];
            ...
            for (uint32_t i = 0; i < number_of_boundaries; i++)
            {
                ...
                switch (boundary_condation_type)
                {
                    case zgCFD::BoundaryCondationType::SPECIFIEDVALUE:
                        ...
                        {
                            int owner = variable.mesh_->faces_[j].owner_;
                            uint32_t index = j - variable.mesh_->number_of_interior_faces_;

                            Scalar fluxff = gamma_f.boundary_value_[index] * variable.mesh_->faces_[j].g_diff_;
                            Scalar fluxvb = fluxff * variable.boundary_condation_[boundary_name].bc_value_ +
                                            (grad_variable_f.boundary_value_[index] & variable.mesh_->faces_[j].t_) *
                                                gamma_f.boundary_value_[index];
                    ...
                    case zgCFD::BoundaryCondationType::SPECIFIEDFLUX:
                        for (uint32_t j = start_face; j < start_face + number_of_faces; j++)
                        {
                            int owner = variable.mesh_->faces_[j].owner_;
                            Scalar fluxvb =
                                variable.mesh_->faces_[j].area_ * variable.boundary_condation_[boundary_name].bc_value_;
                            tmp_matrix.sorce_[owner] -= fluxvb;
                        }
                        break;
                ...
                switch (boundary_condation_type)
                {
                    case zgCFD::BoundaryCondationType::SPECIFIEDVALUE:
                        for (uint32_t j = start_face; j < start_face + number_of_faces; j++)
                        {
                            ...
                            Scalar fluxff = fluxff = gamma_f.boundary_value_[index] * variable.mesh_->faces_[j].g_diff_;
                    ...
                    case zgCFD::BoundaryCondationType::SLIP:
                        for (uint32_t j = start_face; j < start_face + number_of_faces; j++)
                        {
                            ...
                            Scalar fluxff = gamma_f.boundary_value_[index] * variable.mesh_->faces_[j].g_diff_;
                            GeneralVector n = variable.mesh_->faces_[j].sf_ / variable.mesh_->faces_[j].area_;
                            GeneralVector variable_b =
                                variable.interior_value_[owner] - (variable.interior_value_[owner] & n) * n;
                            GeneralVector fluxvb = fluxff * variable_b + (grad_variable_f.boundary_value_[index] &
                                                                          variable.mesh_->faces_[j].t_) *
                                                                             gamma_f.boundary_value_[index];
        ...
        FvmScalarMatrix Convection(ScalarSurfaceField phi, ScalarVolumeField& variable)
        {
            ...
            //                int owner = variable.mesh_->faces_[i].owner_;
            //                uint32_t index =
            //                    i + variable.mesh_->number_of_elements_ - variable.mesh_->number_of_interior_faces_;
            ...
            for (uint32_t i = 0; i < variable.mesh_->number_of_interior_faces_; i++)
            {
                ...
                Scalar gf = variable.mesh_->faces_[i].gf_;

                //线性化系数
                Scalar fluxc = (1 - gf) * phi.interior_value_[i];
                Scalar fluxf = gf * phi.interior_value_[i];

                point p1 = variable.mesh_->faces_[i].centroid_;
                point p2 = variable.mesh_->elements_[owner].centroid_;
                GeneralVector d_cf(p1 - p2);

                Scalar fluxv = 0.5 * phi.interior_value_[i] *
                               ((grad_field.interior_value_[owner] + grad_field_f.interior_value_[i]) & d_cf);
                ...
                switch (boundary_condation_type)
                {
                    case zgCFD::BoundaryCondationType::SPECIFIEDVALUE:
                        for (uint32_t j = start_face; j < start_face + number_of_faces; j++)
                        {
                            ...
                            Scalar fluxv =
                                phi.boundary_value_[index] * variable.boundary_condation_[boundary_name].bc_value_;
                            tmp_matrix.sorce_[owner] -= fluxv;
                        }
                        break;
                    case zgCFD::BoundaryCondationType::SPECIFIEDFLUX:
                    ...
                    case zgCFD::BoundaryCondationType::ZEROGRADIENT:
                        for (uint32_t j = start_face; j < start_face + number_of_faces; j++)
                        {
                            ...
                            Scalar fluxc = phi.boundary_value_[index];
        ...
        FvmVectorMatrix Convection(ScalarSurfaceField phi, VectorVolumeField& variable)
        {
            ...
            grad_field = exp::ComputeGradient(variable, ComputeGradientScheme::GAUSS);
            grad_field_f = exp::InterpolateGradient(grad_field, variable, TheInterpolationScheme::AVERAGE);
            ...
            //    GeneralVector fluxv =
            //        0.5 * phi.value_[i] * ((grad_field.value_[owner] + grad_field_f.value_[i]) & d_cf);
            ...
            //                GeneralVector fluxv = phi.value_[i] *
            //                variable.boundary_condation_[boundary_name].bc_value_; tmp_matrix.sorce_[owner] =
            //                tmp_matrix.sorce_[owner] - fluxv;
            //            }
            //            break;
            //        case zgCFD::BoundaryCondationType::SPECIFIEDFLUX:
        ...
        FvmScalarMatrix Ddt(ScalarVolumeField& variable)
        {
            ...
            for (uint32_t i = 0; i < variable.mesh_->number_of_elements_; i++)
            {
                ...
                tmp_matrix.sorce_[i] += ac_old * variable.interior_value_[i];
            }
            return tmp_matrix;
        }
        ...
        FvmVectorMatrix Ddt(VectorVolumeField& variable)
        {
            ...
            for (uint32_t i = 0; i < variable.mesh_->number_of_elements_; i++)
            {
                ...
                tmp_matrix.sorce_[i] = tmp_matrix.sorce_[i] + ac_old * variable.interior_value_[i];
            }
            return tmp_matrix;
        }
        ...
        FvmScalarMatrix Source(ScalarVolumeField& variable)
        {
            ...
            {
                //线性化系数
                Scalar fluxv = variable.mesh_->elements_[i].volume_ * variable.interior_value_[i];
        ...
        FvmVectorMatrix Source(VectorVolumeField& variable)
        {
            ...
            {
                //线性化系数
                GeneralVector fluxv = variable.mesh_->elements_[i].volume_ * variable.interior_value_[i];
