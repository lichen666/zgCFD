...
namespace zgCFD
{
    ...
    bool OfMesh::ReadOwnerFile()
    {
        ...
        for (int i = 0; i < number_of_owners_; i++)
        {
            ...
            if (tmp_max_of_index < tmp_index_of_owner_)
                tmp_max_of_index = tmp_index_of_owner_;
        }
    ...
    void OfMesh::SetPointConnectinities()
    {
        ...
        for (int i = 0; i < number_of_elements_; i++)
        {
            ...
            for (std::vector<int>::iterator it_face = tmp_faces_of_element_.begin();
                 it_face != tmp_faces_of_element_.end(); it_face++)
            {
                tmp_points_of_face_ = faces_[*it_face].points_;
                for (std::vector<int>::iterator it_point = tmp_points_of_face_.begin();
                     it_point != tmp_points_of_face_.end(); it_point++)
                {
                    for (std::vector<int>::iterator it = elements_[i].points_.begin(); it != elements_[i].points_.end();
                         it++)  //检查是否有重复的点
                    {
                        if (*it == *it_point)
                        {
    ...
    void OfMesh::ProcessOFMesh()
    {
        ...
        for (int i = 0; i < number_of_faces_; i++)
        {
            ...
            else
            {
                ...
                for (std::vector<int>::iterator it = faces_[i].points_.begin(); it != faces_[i].points_.end(); it++)
                {
                    ...
                    point tmp_cemtroid_of_triangle((tmp_point1.x_ + tmp_point2.x_ + tmp_point3.x_) / 3,
                                                   (tmp_point1.y_ + tmp_point2.y_ + tmp_point3.y_) / 3,
                                                   (tmp_point1.z_ + tmp_point2.z_ + tmp_point3.z_) /
                                                       3);  //小三角形的形心
        ...
        for (int i = 0; i < number_of_elements_; i++)
        {
            ...
            for (int i_face = 0; i_face < elements_[i].faces_.size(); i_face++)
            {
                ...
                tmp_centroid_of_pyramid.x_ =
                    0.25 * tmp_centre_of_element.x_ + 0.75 * faces_[elements_[i].faces_[i_face]].centroid_.x_;
                tmp_centroid_of_pyramid.y_ =
                    0.25 * tmp_centre_of_element.y_ + 0.75 * faces_[elements_[i].faces_[i_face]].centroid_.y_;
                tmp_centroid_of_pyramid.z_ =
                    0.25 * tmp_centre_of_element.z_ + 0.75 * faces_[elements_[i].faces_[i_face]].centroid_.z_;
