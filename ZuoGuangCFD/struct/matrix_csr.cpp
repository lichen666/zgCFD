...
namespace zgCFD {
	...
	uint32_t CsrMatrix::GetNumberOfValues(){
		return values_.size();
	}

	void CsrMatrix::SetColumn(uint32_t column)
	{
		number_of_columns_ = column;
	}

	void CsrMatrix::SetRow(uint32_t row)
	{
		number_of_rows_ = row;
		row_offfsets_.resize(row + 1, 0);
	}
