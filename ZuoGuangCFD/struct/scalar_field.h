...
namespace zgCFD
{
    ...
    class ScalarField : public BaseField<Scalar, vol_or_surface>
    {
      public:
        ...
        ScalarField(const Mesh& mesh, const std::string& field_name, const std::string& field_path,
                    const std::string& is_write = "no", const std::string& is_read = "no");
        ScalarField(const Scalar& value, const Mesh& mesh, const std::string& field_name, const std::string& field_path,
                    const std::string& is_write = "no", const std::string& is_read = "no");
...
#include "scalar_fieldl.h"
    ...
    template <int vol_or_surface>
    ScalarField<vol_or_surface>::ScalarField(const Mesh& mesh, const std::string& field_name,
                                             ...
                                             const std::string& is_read)
    {
        ...
        if (vol_or_surface == 0)
        {
            this->value_.resize(mesh.elements_.size() + mesh.number_of_boundary_faces_, 0.);
        }
        else
        {
    ...
    template <int vol_or_surface>
    ScalarField<vol_or_surface>::ScalarField(const Scalar& value, const Mesh& mesh, const std::string& field_name,
                                             const std::string& field_path, const std::string& is_write,
                                             const std::string& is_read)
    {
        // vol_or_surface == 0表示体积场；vol_or_surface == 1表示面积场
    ...
    void ScalarField<vol_or_surface>::ReadField(const std::string& field_name, const std::string& field_path)
    {
        ...
        if (tmp_string == "nonuniform")
        {
            ...
            {
                std::cerr << field_path + "\\" + field_name << "文件内容错误，应为'scalar'。" << std::endl;
                exit(-1);
            }
        }
